...
with torch.no_grad():
  ...
  _,l = z.max(1) # on prend le meilleur score
...
while m<maxIter and torch.any(torch.eq(l,lm)) : # on vérifie qu'il y a au moins 1 label identique (on veut qu'ils soient tous différents à la fin)

  #calcul sur l'image complète
  with torch.no_grad():
    _,lmm=net(xm)["out"][:,classes,:,:].max(1)
  
  # masque = torch.eq(lmm, l)
  # l_s = torch.masked_select(l, masque)
  # l_prime_s = torch.masked_select(l_prime, masque)

  masque = (lmm == l).unsqueeze(1).expand(-1, len(classes), -1, -1)
  print(masque.shape)
  print(xm.shape)
  ...
  #fonction de perte puis gradient
  # loss = adversarialLoss()
  # loss = loss(zm_s, l_s, l_prime)
  # loss.backward()
